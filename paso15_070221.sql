...
SELECT LFOLIO,LFECHA,LHORA,LQUIEN,LCODIG,LDATOA,LDATOB,LUSER FROM ophfo 
...
/*de aqui para adelante es lo nuevo para la vista diaria */
set @fecha_vista_max:= (select max(dfecrea) from opdat_ocupa_cancela_FR);
set @agno_max:=left(@fecha_vista_max,4);

DROP TABLE IF EXISTS fecha_vista_diaria;
CREATE TABLE fecha_vista_diaria
select fecha_uso as fecha_vista
from valle_diccionario.semanas
where semana_temp between -2 and 15
and agno=@agno_max;


DROP TABLE IF EXISTS opdat_ocupa_fvista_diaria;
CREATE TABLE opdat_ocupa_fvista_diaria 
SELECT opdat_ocupa_cancela_FR.*,f.fecha_vista FROM opdat_ocupa_cancela_FR,fecha_vista_diaria f 
WHERE f.fecha_vista>=opdat_ocupa_cancela_FR.dfecrea AND 
(lfechar IS NULL OR lfechar>fecha_vista OR dcance='N') AND
(lfechaf IS NULL OR lfechaf>fecha_vista OR dcance='N');	
CREATE INDEX idx_oo_fvista ON opdat_ocupa_fvista_diaria(dnorsv,dfolio,fecha_vista);
...
DELETE FROM opdat_ocupa_fvista_diaria WHERE fecha_vista>@fecha_vista_max;	

/* 72_create_reserva_vista_adicional */
/* agrego por reserva y fecha de vista los folios con esbase = 0 */

DROP TABLE IF EXISTS reserva_vista_adicional_diaria;
CREATE TABLE reserva_vista_adicional_diaria (dnorsv INT, fecha_vista VARCHAR(8), dadult INT, dninos INT,dinfantes INT,dteen INT, dtarifa FLOAT);
INSERT INTO reserva_vista_adicional_diaria
SELECT dnorsv,fecha_vista, sum(dadult),sum(dninos),sum(dinfantes), sum(dteen), (sum(dtarifa)) 
FROM opdat_ocupa_fvista_diaria WHERE esbase=0 GROUP BY dnorsv,fecha_vista;
CREATE INDEX idx_reserva_vista_adicional ON reserva_vista_adicional_diaria(dnorsv,fecha_vista);
...
DROP TABLE IF EXISTS reserva_folio_vista_diaria;
CREATE TABLE reserva_folio_vista_diaria 
	(dnorsv INT, 
	dfolio INT, 
	...
	dtaras VARCHAR(100),
	dmdepo VARCHAR(100));
INSERT INTO reserva_folio_vista_diaria
	SELECT dnorsv,
			dfolio,
			fecha_vista, 
			min(Nivel),
			...
			group_concat(dtaras),
			group_concat(dmdepo)
FROM opdat_ocupa_fvista_diaria WHERE esbase=1 GROUP BY dnorsv,dfolio,fecha_vista;
create index idx_reserva_folio_vista on reserva_folio_vista_diaria(dnorsv,fecha_vista,DFLLEG,dfolio);
...
drop table if exists reserva_fvista_hab_diaria;
create table reserva_fvista_hab_diaria (dnorsv int, fecha_vista varchar(8), hab int);
insert into reserva_fvista_hab_diaria
	select dnorsv,fecha_vista,sum(esbase) from reserva_folio_vista_diaria group by dnorsv,fecha_vista;
create index idx_reserva_fvista_hab on reserva_fvista_hab_diaria(dnorsv,fecha_vista); 


update reserva_folio_vista_diaria
	set hab= (select hab from reserva_fvista_hab_diaria 
				where reserva_folio_vista_diaria.dnorsv = reserva_fvista_hab_diaria.dnorsv and 
						reserva_folio_vista_diaria.fecha_vista = reserva_fvista_hab_diaria.fecha_vista),
...
drop table if exists reserva_folio_vista_adic_diaria;
create table reserva_folio_vista_adic_diaria like reserva_folio_vista_diaria;
alter table reserva_folio_vista_adic_diaria
...
INSERT INTO reserva_folio_vista_adic_diaria
	SELECT reserva_folio_vista_diaria.*,
		if(reserva_vista_adicional_diaria.dadult,reserva_vista_adicional_diaria.dadult/reserva_folio_vista_diaria.hab,0),
		if(reserva_vista_adicional_diaria.dninos,reserva_vista_adicional_diaria.dninos/reserva_folio_vista_diaria.hab,0),
		if(reserva_vista_adicional_diaria.dinfantes,reserva_vista_adicional_diaria.dinfantes/reserva_folio_vista_diaria.hab,0),
		if(reserva_vista_adicional_diaria.dteen,reserva_vista_adicional_diaria.dteen/reserva_folio_vista_diaria.hab,0),
		if(reserva_vista_adicional_diaria.dtarifa,reserva_vista_adicional_diaria.dtarifa/reserva_folio_vista_diaria.hab,0) 
	FROM reserva_folio_vista_diaria LEFT OUTER JOIN reserva_vista_adicional_diaria
	ON (reserva_folio_vista_diaria.dnorsv=reserva_vista_adicional_diaria.dnorsv AND 
			reserva_folio_vista_diaria.fecha_vista=reserva_vista_adicional_diaria.fecha_vista);
create index idx_reserva_folio_vista_adic on reserva_folio_vista_adic_diaria(dnorsv,fecha_vista,dfolio);



/* 82_expandir_fechas.sql */

DROP TABLE if exists reserva_folio_vista_uso_diaria;
CREATE TABLE reserva_folio_vista_uso_diaria 
SELECT reserva_folio_vista_adic_diaria.*,agno2012.fecha as fecha_uso
from reserva_folio_vista_adic_diaria,agno2012 
where agno2012.fecha>=DFLLEG AND agno2012.fecha<DFSALI;
create index idx_folio_vista_uso on reserva_folio_vista_uso_diaria(dnorsv,dfolio,fecha_vista,fecha_uso);
create index idx_reserva_folio_vista_uso on reserva_folio_vista_uso_diaria(fecha_uso);
...
alter table reserva_folio_vista_uso_diaria
	add column dolar_updated int;

update reserva_folio_vista_uso_diaria
...
set dtarifa=(dtarifa/475.0)/1.19,
	dtarifa_adic=(dtarifa_adic/475.0)/1.19,
	dolar_updated=1
where dmdepo='$' OR (dtarifa>14000 and dnacio='CHI') OR (dtarifa_adic>14000 and dnacio='CHI') OR (dtarifa>100000) or (dtarifa_adic>100000);
...
drop table if exists final_semanas_diaria;
create table final_semanas_diaria
select reserva_folio_vista_uso_diaria.*,semanas.agno,semanas.dia_semana,semanas.semana_temp from reserva_folio_vista_uso_diaria left join semanas on reserva_folio_vista_uso_diaria.fecha_uso = semanas.fecha_uso;
...
drop table if exists final_final_diaria;
create table final_final_diaria
...
max(itau) as itau, max(mastercard) as mastercard, max(directo) as directo, max(early) as early,max(dcance) as dcance, min(fecha_uso) as fecha_uso, max(dagenc) as dagenc, max(dmotiv) as dmotiv, max(dtapor) as dtapor, count(*) as habitaciones_noches, min(nacionalidad_gestion) as nacionalidad_gestion, max(complementary) as complementary from final_semanas_diaria group by dnorsv,dfolio,fecha_vista,agno,semana_temp;
...
drop table if exists final_final_last_diaria;
create table final_final_last_diaria
select *,ceiling(datediff(fecha_vista,fecha_uso)/7) as anticipacion from final_final_diaria;
...
update final_final_last_diaria set nivel='N0' where early=1;
...
DELETE FROM final_final_last_diaria WHERE complementary<>0;


UPDATE final_final_last_diaria
...
CREATE INDEX idx_final ON final_final_last_diaria(fecha_vista, semana_temp, hotel, tipohabrm,nacionalidad_gestion, nivel);
...
drop table if exists final_final_diario_diaria;
create table final_final_diario_diaria
...
sum(dadult)+sum(dadult_adic) + sum(dninos)+sum(dninos_adic) + sum(dteen)+sum(dteen_adic) as noches_cama,
max(dnacio) as dnacio,max(hotel) as hotel, max(tipohabrm) as tipohabrm,max(nivel) as nivel,max(dtaras) as dtaras,max(ibe) as ibe,
max(itau) as itau, max(mastercard) as mastercard, max(directo) as directo, max(early) as early,max(dcance) as dcance, max(dagenc) as dagenc, max(dmotiv) as dmotiv, max(dtapor) as dtapor, count(*) as habitaciones_noches, min(nacionalidad_gestion) as nacionalidad_gestion, max(complementary) as complementary from final_semanas_diaria group by dnorsv,dfolio,fecha_vista,agno,semana_temp, fecha_uso;


drop table if exists final_final_last_diario_diaria;
create table final_final_last_diario_diaria
select *,ceiling(datediff(fecha_vista,fecha_uso)/7) as anticipacion from final_final_diario_diaria;


/* se replican los filtros de final_final_last agregado a pedido LEM, 16 de Agosto 2014 */

update final_final_last_diario_diaria set nivel='N0' where early=1;


DELETE FROM final_final_last_diario_diaria WHERE complementary<>0;


UPDATE final_final_last_diario_diaria
SET nacionalidad_gestion='OTH' WHERE nacionalidad_gestion='OTROS';
...
delete from final_final_last_diario_diaria where agno>2021;


CREATE INDEX idx_final ON final_final_last_diario_diaria(fecha_vista, semana_temp, hotel, tipohabrm,nacionalidad_gestion, nivel, fecha_uso);
CREATE INDEX idx_agenc ON final_final_last_diario_diaria(dagenc);
CREATE INDEX idx_dmotiv ON final_final_last_diario_diaria (dmotiv);
CREATE INDEX idx_dtapor ON final_final_last_diario_diaria (dtapor);
CREATE INDEX idx_pos ON final_final_last_diario_diaria(dnacio);
CREATE INDEX idx_tarifa ON final_final_last_diario_diaria(dtaras);
CREATE INDEX idx_hotel ON final_final_last_diario_diaria(hotel);
CREATE INDEX idx_habitacion ON final_final_last_diario_diaria(tipohabrm);
