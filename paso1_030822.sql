...
set @fecha_ultima_vista:= (select max(DFECREA) from opdat_ocupa_fvista);

DELETE FROM opdat_ocupa_fvista WHERE fecha_vista > @fecha_ultima_vista;	
...
delete from final_final_last_diario where agno>2022;
...
select count(*) from final_final_last;
select count(*) from final_final_last_diario;
